...
core.define('core.models.Order', {
    ...
    backEndValidationErrorMap: {
        ...
        'delivery_address': {
            ...
            '9': {
                attr: ['zipcode','suburb'],
                msg:  'address_not_delivered_to'
            },
            '10': {
                attr: ['zipcode','suburb'],
                msg:  'suburb_zipcode_pair_invalid'
            }
    ...
    */
    ...
    parseBackendValidationErrors: function(response) {
        var backendValidationErrors = {}, parsedErrors = [];
        var me = this;

        if (response.validity) {
            // Handle Order Model Validations
            _.each(response.validity, function(isValid, orderSection, validities){
                var errors = validities.errors;
                if (isValid === false) {
                    backendValidationErrors[orderSection] = errors && errors[orderSection] ? errors[orderSection] : orderSection;
                }
            });

            _.each(backendValidationErrors, function(sectionErrors, orderSection){
                switch (orderSection) {
                    case 'delivery_time':
                    case 'delivery_address':
                        _.each(backendValidationErrors[orderSection], function(errorCode, index){
                            var err = me.backEndValidationErrorMap[orderSection];
                            var errorKey = backendValidationErrors[orderSection][index];
                            if (errorKey !== undefined){
                                parsedErrors.push(err[errorKey]);
                            }
                        });
                    break;
                    case 'payment_method':
                        parsedErrors.push(me.backEndValidationErrorMap.payment_method);
                    break;
                }
            });
        }
        ...
        /**
        * @doc http://mockapi.lieferheld.de/beta/doc/reference/extended_errors.html#reference-response-codes-example
        */
        if (response.errors) {
            _.each(response.errors, function(errorObj, index){
                var errorMessage = errorObj.error_message;
                var errorCode = errorObj.error_code;
                var errorHandler = me.backEndValidationErrorMap.generic[errorCode];
                if (errorHandler){
                    var errorData = errorHandler(errorMessage);
